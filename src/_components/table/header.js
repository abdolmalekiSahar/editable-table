...
export default class Header extends Component {
    ...
    onSort = (event) => {
        let name = event.target.parentNode.innerHTML.toLowerCase().trimLeft();
        this.props.sortColumn(name.slice(0,name.indexOf(" ")));
    }
    ...
    render() {
        ...
        return <thead>
            <tr>
                {columns.map(column => <th key={column.name}>{column.label} {column.required ? '*' : ''} <button onClick={this.onSort}>sort</button>
                </th>)}
                <th>Action</th>
            </tr>
            <tr>
